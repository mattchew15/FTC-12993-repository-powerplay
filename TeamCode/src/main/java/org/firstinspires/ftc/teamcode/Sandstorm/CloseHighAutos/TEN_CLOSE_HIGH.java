...
import static org.firstinspires.ftc.teamcode.Sandstorm.CloseHighAutos.GlobalsCloseHighAuto.outconeStackRotationOtherSide;
...
import static org.firstinspires.ftc.teamcode.Sandstorm.CloseHighAutos.GlobalsCloseHighAuto.outconestackYOtherSide;
...
public class TEN_CLOSE_HIGH extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory DriveOtherSide = drive.trajectoryBuilder(PreloadDrive.end())
               ...
               // .splineToSplineHeading(new Pose2d(outconestackXOtherSide * SideMultiplier,outconestackY, Math.toRadians(180)),Math.toRadians(180))
                .lineTo(new Vector2d(-27, -17))
                .splineToSplineHeading(new Pose2d(-42,-17.7, outconeStackRotationOtherSide),Math.toRadians(180))
                .build();
        ...
        Trajectory ParkLeft = drive.trajectoryBuilder(DriveOtherSide.end())
                .lineTo(new Vector2d(SideMultiplier == 1 ? -45: GlobalsCloseHighAuto.parkRight * SideMultiplier, outconestackY))
                .build();
          ...
          //  telemetry.addData("OtherSide", OtherSide);
            ...
            switch (currentState) {
                        ...
                        if (GlobalTimer.milliseconds() - autoTimer > 100){
                            ...
                            if (GlobalTimer.milliseconds()-autoTimer > 110){
                                ...
                                if ((outtake.intakeArmPosition > 127) && GlobalTimer.milliseconds()-autoTimer > 175){ // this reads the position of the intake arm
                                    ...
                                    if (outtake.intakeSlidePosition > -10 && outtake.intakeArmPosition > 195){ // this controls when the claw closes
                                        outtake.BraceActive();
                                         if (numCycles !=5) {
                                            autoTimer = GlobalTimer.milliseconds(); // reset timer not rly needed here
                ...
                case TURN_OTHER_STACK:
                    holdDrivebaseOtherSide(poseEstimate);
                    ...
                    if (Math.abs(drivebase.getHeadingError()) < Math.toRadians(0.9)){ // have to deal with the heading here, read telemetry for heading angle
                        autoTimer = GlobalTimer.milliseconds(); // reset timer
                        currentState = AutoState.OUTTAKE_CONE;
                        holdDrivebaseOtherSide(poseEstimate); // just so it runs it this loop and doesn't drift??
                        OtherSide = true; // indicates that we are going to do the other stack
                    }
                    break;
                     */
    ...
    public void holdDrivebasePosition(Pose2d currentPose){ // THE OUTCONESTACKROTATION SHOULD BE NEGATIVE
        if (!OtherSide){
            ...
            holdDrivebaseOtherSide(currentPose);
        }
    }

    public void holdDrivebaseOtherSide(Pose2d currentPose){ // this 3 is negative
        ...
        drivebase.DriveToPositionAutonomous3(-GlobalsCloseHighAuto.stackTargetX*SideMultiplier,-22, globalsCloseHighAuto.outconestackXOtherSide * SideMultiplier, globalsCloseHighAuto.outconestackYOtherSide,xPosition,yPosition,headingPosition, 1,1, telemetry, currentPose); // last values are translationalspeed, and rotational speed
    }
