...
public class StormDrive extends LinearOpMode {
    ...
    public void outtakeSequence(){ // make sure to go back and add when each statemachine command should be run
        switch (outtakeState) {
            case READY:
                ...
                //outtake.turretSpin(0,outtake.turretPosition,1); // not this because it takes up battery
            ...
            case LIFT_CONE:
                ...
                if (GlobalTimer.milliseconds() - OuttakeTimer > 105){ // old time was 170
                    ...
                    if (outtake.intakeLiftPosition > IntakeLiftHeightThreshold){ // this is actually the intake lift
                        outtake.IntakeArmTransfer();
                        if ((BeaconScore? GlobalTimer.milliseconds() - OuttakeTimer > 700 :outtake.intakeArmPosition > 197)){ // put the or statement here for the arm being in the right position
            ...
            case CLAW_GRIP_TRANSFER_START: // fix so if i don't go
                intakeClipHoldorNotHold(4);
                if ((outtake.intakeArmPosition > 197) && outtake.intakeLiftPosition > 275 && GlobalTimer.milliseconds() - OuttakeTimer > 92){  // this is actually used as if you are transferring from intake out pickup there is a inbuilt delay - this is an old comment for when there was global timer = 0 - check if intake slidse are all the way in
    ...
    public void IntakeShootOut() {
        switch (intakeout) {
            ...
            case INTAKE_TO_TRANSFER:
                outtake.IntakeClawClose();
                if (GlobalTimer.milliseconds() - IntakeOutTimer > 150){ // wait for the claw to grab
                    outtake.IntakeLiftTransfer();
                    if (GlobalTimer.milliseconds() - IntakeOutTimer > 150){ // if the claw is not picking up from the stack
                        intakeClipHoldorNotHold(56); // hard here
                        if (outtake.intakeLiftPosition > IntakeLiftHeightThreshold){
                            outtake.IntakeArmTransfer();
                        }
                        ...
                        outtake.IntakeSlideTo(150, outtake.intakeSlidePosition,1);
                    }
                } else {
                    outtake.IntakeClipOpen();
                }
        ...
        switch (outtakePickupState) {
            ...
            case READY_TO_OUTTAKE_START:
                if (GlobalTimer.milliseconds() - OuttakePickupTimer > 250){ // time it takes for arm to swing over
                    ...
                    outtake.OuttakeSlideReady();
                    outtake.liftTo(-100,outtake.liftPosition,1);
                }
                break;
            ...
            case GRAB_OUTTAKE: // waiting to for drivers to grab the cone
                ...
                if (!gamepad2RightStickYPastDeadZone()){
                    ...
                    outtake.liftTo(liftTargetPosition, outtake.liftPosition,1); // bumps should be able to be used here to fine adjust
                } else {
                    liftFineAdjust();
                }
                if (gamepad1.right_bumper){
            ...
            case READY: // this case is basically when we want the command in toggle, otherwise we set it to idle case
                ...
                if (GlobalTimer.milliseconds()-OuttakeTimer > 150){ // This is returning the intake
                    ...
                    if (GlobalTimer.milliseconds()-OuttakeTimer > 150){
                        ...
                        if (GlobalTimer.milliseconds()-OuttakeTimer > 500){
                            outtake.IntakeLiftReady();
                            if (gamepad2.right_bumper){ // if the triggers are used for the cone arm dont go into intake
    ...
    public void liftPositionChange(){ // if gamepad inputs don't work in this class then pass them through as parameters in the function
        ...
        } else if (gamepad2.a || gamepad1.a || gamepad2.left_trigger > 0.2){ // if the trigger is pressed then select a height, so that the simaltaneous scoring can still commence
            liftTargetPosition = LiftLowPosition;
            //GroundJunctions = false;
    ...
    public void liftFineAdjust(){ // won't work with the fine adjust from the bumpers, need to make sure the target positions are set on return??
        if (gamepad2RightStickYPastDeadZone()) { // makes the deadzone for the controller more
            if (outtake.liftPosition > LiftUpperLimit && outtake.liftPosition < 1){ // this is so that you can't fine adjust so much that the slide just keeps going and breaks something
    ...
    public double holdHeading(){
            if (HoldHeadingPosition){
               return drivebase.holdHeading(0,correctedHeading,1);
            } else {
                return 0;
            }
    }
    ...
    public boolean gamepad1RightStickXBeingUsed(){
        double gamepadThresholdDistance = 0.17;
        if (gamepad1.right_stick_x > gamepadThresholdDistance || gamepad1.right_stick_x < -gamepadThresholdDistance) { // makes the deadzone for the controller more
            return true;
        } else {
            return false;
        }
